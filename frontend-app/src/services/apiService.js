...
// Força uso de mock para desenvolvimento
...
const fetchWithTimeout = async (url, options = {}) => {
  try {
    ...
    try {
      ...
      clearTimeout(id);
      console.error('[ApiService] Erro em fetch:', error);
      throw error;
    }
  } catch (error) {
    console.error('[ApiService] Erro em fetchWithTimeout:', error);
    throw error;
  }
};
...
const request = async (url, options = {}) => {
  try {
    // Se estamos forçando o uso de mock para desenvolvimento, interrompe a requisição
    if (FORCE_MOCK) {
      console.log('[ApiService] Usando modo mock para:', url);
      throw new Error('Usando modo mock');
    }
    ...
    try {
      ...
      console.warn('[ApiService] Erro ao acessar AsyncStorage:', error);
    }
    ...
    // Preparar URL completa - tratando undefined e null
    let fullUrl;
    try {
      fullUrl = url && url.startsWith && url.startsWith('http')
        ? url 
        : `${API_CONFIG.baseURL}${url && url.startsWith && url.startsWith('/') ? url : `/${url || ''}`}`;
    } catch (error) {
      console.error('[ApiService] Erro ao formatar URL:', error);
      fullUrl = `${API_CONFIG.baseURL}/`;
    }
    ...
    if (!response.ok) {
      ...
      try {
        ...
        console.warn('[ApiService] Erro ao processar resposta JSON:', error);
      }
    ...
    if (error.name === 'AbortError') {
      console.warn('[ApiService] Requisição abortada por timeout');
      throw new Error('A requisição excedeu o tempo limite. Tente novamente.');
    }
    
    if (error.message === 'Network request failed') {
      console.warn('[ApiService] Falha na requisição de rede');
      throw new Error('Não foi possível conectar ao servidor. Verifique sua conexão.');
    }
    
    if (error.message === 'Usando modo mock') {
      // Silenciosamente continua para usar mockService
      console.log('[ApiService] Continuando com modo mock');
      return null;
    }
...
export class ApiService {
  ...
  static async initialize() {
    try {
      ...
      // Ainda retorna true para não bloquear o carregamento do app
      return true;
    }
  }
